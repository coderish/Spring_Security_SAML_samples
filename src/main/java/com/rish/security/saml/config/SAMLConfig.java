package com.rish.security.saml.config;
...
import com.rish.security.saml.certificate.KeystoreFactory;
import com.rish.security.saml.spring.SpringResourceWrapperOpenSAMLResource;
import com.rish.security.saml.spring.security.SAMLUserDetailsServiceImpl;

@AutoConfigureBefore(WebSecurityConfig.class)
@Configuration
public class SAMLConfig {
//	private static final Logger log = Logger.getLogger(SAMLConfig.class);
			...
			try {
				...
				Stream.of(idpMetadataFiles).forEach(idpMetadataFile -> {
					try {
						...
						delegate = new ResourceBackedMetadataProvider(refreshTimer,
						...
						String idpName = idpFileName.substring(idpFileName.lastIndexOf("idp-") + 4,
								idpFileName.lastIndexOf(".xml"));
...
//						log.info("Loaded Idp Metadata bean {}: {}", idpName, idpMetadataFile);
	...
	public KeyManager keyManager(KeystoreFactory keystoreFactory) throws Exception {
		KeyStore keystore = keystoreFactory.loadKeystore("classpath:/localhost.cert", "classpath:/localhost.key.der",
				"localhost", "");
		Map<String, String> map = new HashMap<>();
		map.put("localhost", "");
		return new JKSKeyManager(keystore, Collections.unmodifiableMap(map), "localhost");
	}
